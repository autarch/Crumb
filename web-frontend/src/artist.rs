use crate::{
    client::{get_artist_response, Client, ReleaseListItem},
...
pub(crate) fn Artist(cx: Scope) -> Element {
    ...
    let artist = use_future(
        &cx,
        || async move { Client::new().get_artist(&artist_id).await },
    );
    cx.render(rsx! {
        crate::Crumb {
            match artist.value() {
...
fn OneRelease<'a>(cx: Scope, release: &'a ReleaseListItem) -> Element {
    ...
    let year = release.best_release_year();
    let track_count = crate::util::maybe_plural(release.track_count, "track");
    cx.render(rsx! {
        div {
            ...
            div {
                ...
                Link {
                    to: "{release_url}",
                    AlbumCover {
                        uri: release.album_cover_uri.as_deref(),
                        ring: true,
                    },
                },
            },
